            ...
            <div className="flex-1 flex flex-col lg:flex-row max-w-7xl mx-auto w-full px-6 py-10 gap-10">
                ...
                <aside className="lg:w-72 flex flex-col gap-8">
                        ...
                        <nav className="space-y-2">
                            {categories.map((cat) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all font-bold text-sm ${selectedCategory === cat.id
                                        ? 'bg-[#2E7D32] text-white shadow-lg shadow-green-900/10'
                                        : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
                                        }`}
                                >
                                    <span className="text-xl">{cat.icon}</span>
                                    <span>{t(`hub.category.${cat.id}`)}</span>
                                </button>
                            ))}
                        </nav>
                    </div>
                    ...
                    <div className="bg-[#0F4C3A] rounded-[32px] p-8 text-white shadow-xl shadow-green-900/10 relative overflow-hidden group">
                        ...
                        <h3 className="text-lg font-bold mb-4 leading-tight">{t('hub.masterclass.title')}</h3>
                        <p className="text-sm text-green-50/70 font-medium mb-8">{t('hub.masterclass.desc')}</p>
